...
```bash
git clone https://github.com/MaxNikitenok/nodejs-crud-api.git
```
```bash
git checkout dev
```
...
Run the application in development mode:
   ...
   npm run start:dev
   ```
   ## !!! app only runs in dev mode !!! because someone really likes to sleep
   
   This command will run the application in development mode with `nodemon` and `ts-node` packages on a port from `.env`.

   By default, it will run on http://localhost:4001/api/users.
